...
function db = Hprepare_database(src,feature_fun,opt,SNR)
	if nargin < 3
		opt = struct();
	end
	...
	opt = fill_struct(opt, 'parallel',1); %hao it was 1, but I did not like to do parallel compute  
...
% 			x = data_read(src.files{k});    
            ...
            % upsampling
%             x=interp(double(real(x)),2)+1i*interp(double(imag(x)),2);
%             % shift to positive frequence
%             fx=fft(x);
%             x=ifft(circshift(fx,length(fx)/4));
            % flip
%             x=ifft(flip(fft(x)));
            %adding noise    
